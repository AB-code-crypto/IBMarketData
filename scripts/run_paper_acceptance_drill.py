...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run one non-interactive paper acceptance sequence: prepare LONG/SHORT, "
            "submit one MARKET entry, prove idempotency and broker position, create "
            "the position episode, submit STOP first, then TAKE PROFIT, and prove "
            "that repeated invocations do not create duplicate broker orders."
        )
    )
    ...
    parser.add_argument("--entry-max-invocations", type=int, default=8)
    parser.add_argument("--entry-poll-seconds", type=float, default=1.0)
    parser.add_argument("--position-wait-seconds", type=float, default=45.0)
    parser.add_argument("--position-poll-seconds", type=float, default=1.0)
    parser.add_argument("--protective-max-invocations", type=int, default=12)
...
def _drill_id(value: str | None) -> str:
    ...
    return "paper-acceptance-" + utc_now().strftime("%Y%m%dT%H%M%SZ")
...
def _validation_payload(
    *,
    ...
    return {
        "paper_acceptance_dependencies_compatible": True,
...
def run(arguments: argparse.Namespace) -> int:
    ...
    artifacts_root = (
        ...
        else settings.data_root / "runtime" / "paper_acceptance"
    )
    ...
    runner = PaperAcceptanceDrillRunner(
        policy=policy,
    ...
    try:
        ...
        failure = {
            "schema_name": "PaperAcceptanceFailure",
            "schema_version": 1,
            "drill_id": drill_id,
            ...
            "required_operator_action": (
                "Inspect TWS and close MNQ manually if position_may_be_open=true. "
                "Do not create a new drill_id until the existing broker state is "
                "reconciled."
            ),
        }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if int(bool(arguments.validate_only)) + int(bool(arguments.run)) != 1:
        print(
            "paper acceptance requires exactly one mode: --validate-only or --run",
            file=sys.stderr,
        )
        return 2
    try:
        ...
        print(
            f"paper acceptance failed before runner startup: "
