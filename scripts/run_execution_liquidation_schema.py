...
class ExecutionLiquidationSchemaError(RuntimeError):
    pass
...
def _load_manifest(path: Path) -> tuple[str, int, int, tuple[str, ...], str]:
    try:
        ...
        raise ExecutionLiquidationSchemaError(
            f"cannot read execution liquidation manifest {path}: {exc}"
        ) from exc
    if not isinstance(value, dict) or set(value) != _EXPECTED_KEYS:
        actual = set(value) if isinstance(value, dict) else set()
        raise ExecutionLiquidationSchemaError(
            "execution liquidation manifest fields mismatch: "
    ...
    if not isinstance(statements, list) or not statements or not all(
        ...
        raise ExecutionLiquidationSchemaError(
            "component statements must be a non-empty list of SQL strings"
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect or explicitly install the execution liquidation schema "
            "component. The default is read-only."
        )
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default=ROOT / "migrations" / "execution.liquidation.v1.json",
    )
    ...
    if not application_version:
        raise ExecutionLiquidationSchemaError("application_version is required")
    ...
    try:
        ...
        if existing is not None:
            ...
            if stored_version != version or stored_checksum != checksum:
                raise ExecutionLiquidationSchemaError(
                    "liquidation component changed in place; create a fresh target "
                    "execution database instead of migrating development data"
                )
            print(
                json.dumps(
                    _payload(
...
if __name__ == "__main__":
    ...
    except ExecutionLiquidationSchemaError as exc:
        print(f"execution liquidation schema failed: {exc}", file=sys.stderr)
