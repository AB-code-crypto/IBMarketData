...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run one non-interactive paper REVERSE acceptance: stage the "
            "opposite command, cancel old TP then STOP, submit one MARKET "
            "reverse, split close/open fills, create the opposite position "
            "episode, install new STOP/TP and prove idempotency."
        )
    )
    ...
    parser.add_argument(
        ...
        choices=("LONG", "SHORT"),
        default=None,
        help="optional; by default the opposite side is derived from source proof",
    )
    ...
    parser.add_argument("--handoff-max-invocations", type=int, default=12)
    parser.add_argument("--handoff-poll-seconds", type=float, default=1.0)
...
def _validate_stores(paths: PaperAcceptancePathsV1) -> None:
    validators = (
        ...
        SQLiteLiquidationStore(paths.execution_database),
        SQLiteReverseFinalizationStore(paths.execution_database),
...
def _source_target(
    ...
    value = read_json_object(source_summary)
    if value.get("schema_name") not in {
        "PaperAcceptanceResult",
        "PaperRestartAcceptanceResult",
    } or int(value.get("schema_version") or 0) != 1:
        raise ValueError(
            "--source-summary must be PaperAcceptanceResult or "
            "PaperRestartAcceptanceResult v1"
        )
    proof = value.get("position_proof")
    if not isinstance(proof, dict) or proof.get("accepted") is not True:
        raise ValueError("source summary does not contain accepted position proof")
    signed = float(proof.get("signed_quantity") or 0.0)
    if signed > 0.0:
        derived = "SHORT"
    elif signed < 0.0:
        derived = "LONG"
    else:
        raise ValueError("source summary position proof has zero quantity")
    if requested is not None and requested != derived:
        raise ValueError(
            f"requested target side is not opposite: derived={derived}, "
...
def _drill_id(value: str | None) -> str:
    ...
    return "paper-reverse-" + utc_now().strftime("%Y%m%dT%H%M%SZ")


def run(arguments: argparse.Namespace) -> int:
    settings = load_deployment_settings()
    paths = _paths(arguments, settings.data_root)
    _validate_stores(paths)
    if arguments.validate_only:
        print(
            json.dumps(
                {
                    "paper_reverse_acceptance_dependencies_compatible": True,
    ...
    _source_value, target_side = _source_target(
        source_summary,
    ...
    artifacts_root = (
        ...
        else settings.data_root / "runtime" / "paper_reverse_acceptance"
    )
    ...
    runner = PaperReverseAcceptanceRunner(
        ...
        state_source=state,
        artifacts=artifacts,
        handoff_max_invocations=arguments.handoff_max_invocations,
        handoff_poll_seconds=arguments.handoff_poll_seconds,
        sleeper=time.sleep,
    )
    try:
        result = runner.run()
        ...
        failure = {
            "schema_name": "PaperReverseAcceptanceFailure",
            "schema_version": 1,
            "drill_id": drill_id,
            "target_side": target_side,
            ...
            "required_operator_action": (
                "Inspect TWS and saved artifacts. Do not create another REVERSE "
                "command while the current operation or old protective orders "
                "remain unresolved."
            ),
        }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if int(bool(arguments.validate_only)) + int(bool(arguments.run)) != 1:
        print(
            "paper reverse acceptance requires exactly one mode: "
    ...
    try:
        ...
        print(
            "paper reverse acceptance failed before runner startup: "
