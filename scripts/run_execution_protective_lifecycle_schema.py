...
class ExecutionComponentSchemaError(RuntimeError):
    pass
...
def _load_manifest(path: Path) -> tuple[str, int, int, tuple[str, ...], str]:
    try:
        ...
        raise ExecutionComponentSchemaError(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect or explicitly install the execution protective-lifecycle "
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=ROOT / "migrations" / "execution.protective_lifecycle.v1.json",
    )
    ...
    if not application_version:
        raise ExecutionComponentSchemaError("application_version is required")
    ...
    try:
        ...
        if existing is not None:
            ...
            if stored_version != version or stored_checksum != checksum:
                raise ExecutionComponentSchemaError(
                    "protective lifecycle component changed in place; create a fresh "
                    "target execution database instead of migrating legacy data"
                )
            print(
                json.dumps(
                    _payload(
...
if __name__ == "__main__":
    ...
    except ExecutionComponentSchemaError as exc:
        print(f"execution protective lifecycle schema failed: {exc}", file=sys.stderr)
