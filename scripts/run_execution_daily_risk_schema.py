...
class DailyRiskSchemaError(RuntimeError):
    pass
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect or explicitly install the execution daily-risk schema "
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=ROOT / "migrations" / "execution.daily_risk.v1.json",
    )
    ...
    if not application_version:
        raise DailyRiskSchemaError("application_version is required")
    ...
    try:
        ...
        if existing is not None:
            ...
            if stored_version != version or stored_checksum != checksum:
                raise DailyRiskSchemaError(
                    "daily-risk component changed in place; create a fresh target "
...
if __name__ == "__main__":
    ...
    except DailyRiskSchemaError as exc:
        print(
            f"execution daily-risk schema failed: {exc}",
            file=sys.stderr,
        )
