...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run one non-interactive paper liquidation acceptance sequence from a "
            "successful entry/protection summary: request one durable liquidation, "
            "cancel TAKE PROFIT, cancel STOP, submit one MARKET close, prove broker "
            "FLAT and prove that a repeated invocation creates no broker mutation."
        )
    )
...
def _validation_payload(
    *,
    ...
    return {
        "paper_liquidation_acceptance_dependencies_compatible": True,
        ...
        "expected_fresh_broker_mutations": [
            "CANCEL_TAKE_PROFIT",
            "CANCEL_STOP",
            "SUBMIT_MARKET_CLOSE",
        ],
...
def run(arguments: argparse.Namespace) -> int:
    ...
    artifacts_root = (
        ...
        else settings.data_root / "runtime" / "paper_liquidation_acceptance"
    )
    ...
    try:
        ...
        failure = {
            "schema_name": "PaperLiquidationAcceptanceFailure",
            "schema_version": 1,
            ...
            "required_operator_action": (
                "Inspect TWS and target evidence immediately when "
                "broker_exposure_possible=true. Do not submit another liquidation "
                "or create another target position until the existing operation is "
                "reconciled."
            ),
        }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if int(bool(arguments.validate_only)) + int(bool(arguments.run)) != 1:
        print(
            "paper liquidation acceptance requires exactly one mode: "
    ...
    try:
        ...
        print(
            "paper liquidation acceptance failed before runner startup: "
